import { Result, Option, unwrap, Ok, Err, CustomError, errors, Message } from "./prelude"
...
abstract class ToValue {
    abstract equals(other: this, symbols: Readonly<Record<string, Symbol>>): boolean
    abstract get_result(symbols: Readonly<Record<string, Symbol>>): Result<LispResult>
    abstract serialize(): any
}

abstract class ToText extends ToValue {
    abstract get_text(symbols: Readonly<Record<string, Symbol>>): Result<Text>
}

abstract class ToNumber extends ToText {
    abstract get_number(symbols: Readonly<Record<string, Symbol>>): Result<Num>
}

abstract class ToDecimal extends ToText {
    abstract get_number(symbols: Readonly<Record<string, Symbol>>): Result<Num>
    abstract get_decimal(symbols: Readonly<Record<string, Symbol>>): Result<Decimal>
}

abstract class ToBoolean extends ToText {
    abstract get_boolean(symbols: Readonly<Record<string, Symbol>>): Result<Bool>
}

class Num implements ToNumber, ToDecimal, ToText {
    value: number
...
class Decimal implements ToNumber, ToDecimal, ToText {
    value: number
...
class Bool implements ToBoolean, ToText {
    value: boolean
...
function fold<T, U>(init: T, values: ReadonlyArray<U>, fn: (acc: T, val: U) => T): T {
    let acc: T = init
...
function fold_prev<T, U>(init: T, prev: U, values: ReadonlyArray<U>, fn: (acc: T, prev: U, val: U) => T): T {
    let acc: T = init
    for (let val of values) {
        acc = fn(acc, prev, val)
        prev = val
...
type ArithmeticExpressionVariant<T extends ToNumber | ToDecimal> = Add<T> | Multiply<T> | Subtract<T> | Divide<T> | Modulus<T>

class NumberArithmeticExpression implements ToNumber, ToDecimal {
    ...
    eval(symbols: Readonly<Record<string, Symbol>>): Result<ToNumber> {
        ...
        if (this.value instanceof Add) {
            let result: Result<ToNumber> = fold(args[0].get_number(symbols), args[1], (acc, val) => {
                if (unwrap(acc)) {
...
class DecimalArithmeticExpression implements ToNumber, ToDecimal {
    ...
    eval(symbols: Readonly<Record<string, Symbol>>): Result<ToDecimal> {
        ...
        if (this.value instanceof Add) {
            let result: Result<ToDecimal> = fold(args[0].get_decimal(symbols), args[1], (acc, val) => {
                if (unwrap(acc)) {
...
type ComparatorExpressionVariant<T> = Equals<T> | GreaterThan<T> | LessThan<T> | GreaterThanEquals<T> | LessThanEquals<T>

class NumberComparatorExpression implements ToBoolean {
    ...
    eval(symbols: Readonly<Record<string, Symbol>>): Result<ToBoolean> {
        ...
        if (this.value instanceof Equals) {
            ...
            if (unwrap(v)) {
                ...
                if (unwrap(v1)) {
                    let result: Result<Bool> = fold_prev(new Ok(new Bool(v.value.value === v1.value.value)) as Result<Bool>, args[1], args[2], (acc, prev, val) => {
                        if (unwrap(acc)) {
...
class DecimalComparatorExpression implements ToBoolean {
    ...
    eval(symbols: Readonly<Record<string, Symbol>>): Result<ToBoolean> {
        let args: [ToDecimal, ToDecimal, ReadonlyArray<ToDecimal>] = this.value.value
        if (this.value instanceof Equals) {
...
class TextComparatorExpression implements ToBoolean {
...
class LogicalBinaryExpression implements ToBoolean {
    ...
    eval(symbols: Readonly<Record<string, Symbol>>): Result<ToBoolean> {
        let args: [ToBoolean, ToBoolean, ReadonlyArray<ToBoolean>] = this.value.value
...
class LogicalUnaryExpression implements ToBoolean {
...
class MatchExpression<T extends ToValue, U extends ToValue> implements ToNumber, ToDecimal, ToText, ToBoolean {
    value: Match<T, U>
        ...
        if (unwrap(condition)) {
            for (let guard of args[1]) {
                ...
                if (unwrap(v)) {
                    ...
                    } else if (condition.value instanceof Decimal && v.value instanceof Decimal) {
                        if (condition.value.equals(v.value, symbols)) {
                            return guard[1].get_result(symbols)
                        }
                    } else if (condition.value instanceof Text && v.value instanceof Text) {
                        if (condition.value.equals(v.value, symbols)) {
                            return guard[1].get_result(symbols)
                        }
                    } else if (condition.value instanceof Bool && v.value instanceof Bool) {
                        if (condition.value.equals(v.value, symbols)) {
                            return guard[1].get_result(symbols)
                        }
                    }
...
class DotExpression implements ToNumber, ToText, ToBoolean {
    value: Dot
    ...
    get_leaf(path: ReadonlyArray<string>, symbols: Readonly<Record<string, Symbol>>): Result<Leaf> {
        if (path.length !== 0) {
            let v = path[0]
            if (v in symbols) {
